...
def preprocess_dataset_for_resnet(x,y,width=32,height=32):
    ...
    from jet_ml.dataset import is_normalized,convert_to_rgb,resize_images,resize_images_by_tensor_in_batches,resize_images_by_sliced_tensor_in_batches,resize_y
    display("Data is normalized: {0}".format(is_normalized(x)))

    x_rgb=convert_to_rgb(x)
    display("x_rgb.shape {0}".format(x_rgb.shape))
    
    x_resized=resize_images_by_sliced_tensor_in_batches(x_rgb,width,height)
